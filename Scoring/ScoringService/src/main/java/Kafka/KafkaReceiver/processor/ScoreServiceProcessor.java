...
public class ScoreServiceProcessor {
    ...
    public void processChanges(StreamsBuilder streamsBuilder) {
        ...
        KTable<Windowed<String>, String> windowedStreamX = streamsBuilder
                .stream(inputX, Consumed.with(Serdes.String(), Serdes.String()))
                .groupByKey()
        ...
        KTable<Windowed<String>, String> windowedStreamY = streamsBuilder
                .stream(inputY, Consumed.with(Serdes.String(), Serdes.String()))
                .groupByKey()
                .windowedBy(TimeWindows.of(windowSize))
                .reduce((v1, v2) -> {
                    int val = Integer.parseInt(v1) + Integer.parseInt(v2);
        ...
        windowedStreamX
                ...
                .toStream()
                // .peek((k, v)-> System.out.println("wi: "+ Arrays.toString(v) + "K: "+
                // k.toString()))
                // 3rd stage
                .map((k, v) -> {
                    int interaction = 0;
